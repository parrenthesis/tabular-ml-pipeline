...
from src.predict import find_threshold_for_precision, predict, predict_with_threshold
...
def test_find_threshold_for_precision():
    ...
    threshold, precision, recall = find_threshold_for_precision(
        y_true, y_proba, target_precision=0.7
    )
...
def test_find_threshold_for_precision_no_match():
    ...
    threshold, precision, recall = find_threshold_for_precision(
        y_true, y_proba, target_precision=1.1
    )
...
def test_find_threshold_for_precision_idx_edge():
    ...
    threshold, precision, recall = find_threshold_for_precision(
        y_true, y_proba, target_precision=1.0
    )
    assert isinstance(threshold, float)
    assert isinstance(precision, float)
    assert isinstance(recall, float)
