...
import numpy as np
import logging
from src.utils import plot_feature_importances, plot_precision_recall_curve, setup_logging
...
def test_plot_feature_importances():
    ...
    path2 = plot_feature_importances(importances, feat_names, 'TestModel', show=True, save=False)
    assert path2 is None
    # Show and save
    path3 = plot_feature_importances(importances, feat_names, 'TestModel', show=True, save=True)
...
def test_plot_precision_recall_curve():
    ...
    assert path is not None and path.endswith('.png')
    # Show only (should not save)
    path2 = plot_precision_recall_curve(precisions, recalls, thresholds, 'TestModel', show=True, save=False)
    assert path2 is None
    # Show and save
    path3 = plot_precision_recall_curve(precisions, recalls, thresholds, 'TestModel', show=True, save=True)
    assert path3 is not None and path3.endswith('.png')
    plt.close('all') 