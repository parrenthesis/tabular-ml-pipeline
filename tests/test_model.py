...
def test_train_and_tune_model_rf():
    ...
    model, results = train_and_tune_model(
        X, y, numeric, categorical, model_type="rf", cv_splits=2
    )
...
def test_train_and_tune_model_xgb():
    X, y, numeric, categorical = make_toy_data()
    model, results = train_and_tune_model(
        X, y, numeric, categorical, model_type="xgb", cv_splits=2
    )
    assert hasattr(model, "predict")
    assert "best_params" in results
    assert "roc_auc" in results
...
def test_train_and_tune_model_invalid_type():
    ...
    with pytest.raises(ValueError):
        train_and_tune_model(
            X, y, numeric, categorical, model_type="invalid", cv_splits=2
        )
