...
from src.predict import find_threshold_for_precision
...
def run_pipeline(db_path, model, output_dir, log_dir):
    ...
    logger.info(
        f"Train class balance: {y_train.value_counts(normalize=True).to_dict()}"
    )
    ...
    for name, model in model_objs.items():
        ...
        thresh, prec, rec = find_threshold_for_precision(
            y_test.values, y_proba, target_precision
        )
        y_pred_auto = (y_proba >= thresh).astype(int)
        logger.info(
            f"{name} Auto-tuned threshold for precision>={target_precision}: "
            f"threshold={thresh:.3f}, Precision={prec:.3f}, Recall={rec:.3f}, "
            f"Positives={np.sum(y_pred_auto == 1)}"
        )
        # Default threshold
        y_pred_default = (y_proba >= 0.5).astype(int)
        def_prec = precision_score(y_test, y_pred_default)
        def_recall = np.sum((y_pred_default == 1) & (y_test == 1)) / max(
            np.sum(y_test == 1), 1
        )
        logger.info(
            f"{name} Default threshold=0.5: Precision={def_prec:.3f}, "
            f"Recall={def_recall:.3f}, Positives={np.sum(y_pred_default == 1)}"
        )


def main():
    parser = argparse.ArgumentParser(
        description="Tabular ML Pipeline: Predicting Treatment Need (Synthetic Healthcare Data)"
    )
    parser.add_argument(
        "--db-path",
        type=str,
        default=os.path.join("data", "data.db"),
    ...
    parser.add_argument(
        ...
        help="Model to train (rf, xgb, or all)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
